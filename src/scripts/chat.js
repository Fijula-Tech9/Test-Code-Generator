...
class ChatUI {
  ...
  initialize() {
    ...
    chrome.storage.sync.get(
      ...
      (result) => {
        ...
        if (result.openaiApiKey)
          this.openaiAPI = new OpenAIAPI(result.openaiApiKey);
    ...
    chrome.storage.onChanged.addListener((changes) => {
      if (changes.groqApiKey)
        this.groqAPI = new GroqAPI(changes.groqApiKey.newValue);
      if (changes.openaiApiKey)
        this.openaiAPI = new OpenAIAPI(changes.openaiApiKey.newValue);
      if (changes.selectedModel)
        this.selectedModel = changes.selectedModel.newValue;
      if (changes.selectedProvider)
        this.selectedProvider = changes.selectedProvider.newValue;
    });
    ...
    this.inspectorButton.addEventListener("click", async () => {
      try {
        ...
        this.addMessage(
          "Failed to activate inspector. Please refresh and try again.",
          "system"
        );
        this.isInspecting = false;
        this.updateInspectorButtonState();
  ...
  initializeDownloadButton() {
    ...
    downloadBtn.addEventListener("click", () => {
      ...
      let finalCode = this.generatedCode
        .replace(/^```(?:java|typescript|javascript|gherkin)?\s*/i, "")
        .replace(/```\s*$/i, "");

      let fileName = "";
      // Determine file name based on generator type and extract class/feature name if available
      if (
        this.codeGeneratorType.includes("PAGE_ONLY") ||
        this.codeGeneratorType.includes("TEST_ONLY")
      ) {
        // Try to extract class name from the generated code
        const match = finalCode.match(/(?:class|public class)\s+(\w+)/);
        fileName = match ? match[1] : "Generated";
        if (this.codeGeneratorType.includes("TS")) {
          fileName += ".ts";
        } else if (this.codeGeneratorType.includes("JS")) {
          fileName += ".js";
        } else {
          fileName += ".java";
        }
      } else if (this.codeGeneratorType.includes("CUCUMBER")) {
  ...
  async sendMessage() {
    ...
    if (!apiRef) {
      this.addMessage(
        `Please set your ${this.selectedProvider} API key in the Settings tab.`,
        "system"
      );
      return;
    }
    ...
    try {
      ...
      const finalSnippet =
        typeof this.selectedDomContent === "string"
      ...
      for (const key of promptKeys) {
        ...
        const resp = await apiRef.sendMessage(builtPrompt, this.selectedModel);
      ...
      const loader = this.messagesContainer.querySelector(
  ...
  addMessage(content, type) {
    ...
    if (type === "user") {
      ...
      const genType = this.codeGeneratorType.includes("PLAYWRIGHT")
        ? "Playwright"
        : "Selenium";
      loader.innerHTML = `
              <div class="loading-spinner"></div>
    ...
    const msgCount =
      this.messagesContainer.querySelectorAll(".chat-message").length;
  ...
  addMessageWithMetadata(content, type, metadata) {
    ...
    copyBtn.onclick = () => {
      ...
      let combinedCode = Array.from(codeBlocks)
  ...
  getPromptKeys(language, engine) {
    const checkboxes = Array.from(
      document.querySelectorAll('input[name="javaGenerationMode"]:checked')
    );
    const promptKeys = [];
    ...
    // Java + Selenium
    if (lang === "java" && eng === "selenium") {
      if (isFeatureChecked && isPageChecked) {
        promptKeys.push("CUCUMBER_WITH_STEPS");
    ...
    // Java + Playwright
    if (lang === "java" && eng === "playwright") {
      if (isFeatureChecked && isPageChecked) {
        promptKeys.push("CUCUMBER_WITH_STEPS");
        promptKeys.push("PLAYWRIGHT_JAVA_PAGE_ONLY");
      } else if (isFeatureChecked) {
        promptKeys.push("CUCUMBER_ONLY");
      } else if (isPageChecked) {
        promptKeys.push("PLAYWRIGHT_JAVA_PAGE_ONLY");
    ...
    if (lang === "javascript" && eng === "playwright") {
      if (isFeatureChecked && isPageChecked) {
        promptKeys.push("CUCUMBER_ONLY");
        promptKeys.push("PLAYWRIGHT_JS_PAGE_ONLY");
      } else if (isFeatureChecked) {
        promptKeys.push("CUCUMBER_ONLY");
      } else if (isPageChecked) {
        promptKeys.push("PLAYWRIGHT_JS_PAGE_ONLY");
    ...
    if (lang === "typescript" && eng === "playwright") {
      if (isFeatureChecked && isPageChecked) {
        promptKeys.push("CUCUMBER_ONLY");
    ...
    // JavaScript + Cypress
    if (lang === "javascript" && eng === "cypress") {
      if (isFeatureChecked && isPageChecked) {
        promptKeys.push("CUCUMBER_ONLY");
        promptKeys.push("CYPRESS_JS_PAGE_ONLY");
      } else if (isFeatureChecked) {
        promptKeys.push("CUCUMBER_ONLY");
      } else if (isPageChecked) {
        promptKeys.push("CYPRESS_JS_PAGE_ONLY");
    ...
    // TypeScript + Cypress
    if (lang === "typescript" && eng === "cypress") {
      if (isFeatureChecked && isPageChecked) {
        promptKeys.push("CUCUMBER_ONLY");
        promptKeys.push("CYPRESS_TS_PAGE_ONLY");
      } else if (isFeatureChecked) {
        promptKeys.push("CUCUMBER_ONLY");
      } else if (isPageChecked) {
        promptKeys.push("CYPRESS_TS_PAGE_ONLY");
    ...
    if (promptKeys.length === 0) {
      if (lang === "javascript" && eng === "playwright") {
        ...
        promptKeys.push("PLAYWRIGHT_TS_PAGE_ONLY");
      } else if (lang === "javascript" && eng === "cypress") {
        promptKeys.push("CYPRESS_JS_PAGE_ONLY");
      } else if (lang === "typescript" && eng === "cypress") {
        promptKeys.push("CYPRESS_TS_PAGE_ONLY");
      } else if (lang === "java" && eng === "selenium") {
        promptKeys.push("SELENIUM_JAVA_PAGE_ONLY");
      } else if (lang === "java" && eng === "playwright") {
        promptKeys.push("PLAYWRIGHT_JAVA_PAGE_ONLY");
      } else {
        promptKeys.push("PLAYWRIGHT_JS_PAGE_ONLY");
      }
  ...
  async initializeCodeGeneratorType() {
    const { codeGeneratorType } = await chrome.storage.sync.get([
      "codeGeneratorType",
    ]);
  ...
  async initializeTokenThreshold() {
    const { tokenWarningThreshold } = await chrome.storage.sync.get([
  ...
  async resetChat() {
    try {
      ...
      this.addMessage(
        "Error resetting chat. Please close and reopen.",
        "system"
      );
    }
  }
